    ...
    <!-- <div class="king-page-topbar pl20 pr20 "> -->
        <div class="king-page-topbar ">
            ...
            <ul class="breadcrumb king-breadcrumb pl0 bg-transparent">
                ...
                <li class="active"><a href="/secondStep">水体提取</a></li>
            </ul>
        </div>
        <!-- 面板 编辑 开始 -->
        <!-- 面板 编辑 结束 -->
        <div class="king-block king-block-bordered king-block-themed mb0">
            <div class="king-block-header king-gray-dark">
                ...
                <h3 class="king-block-title">第二步 水体检测</h3>
            </div>
            <div class="king-block-content">
                <form class="form-horizontal" method="POST" style="padding-left: 14%;padding-right: 14%">
                    <div class="form-group">
                        <label for="inputBandGreen" class="col-sm-3 control-label">绿波段序号：</label>
                        <div class="col-sm-7">
                            <input type="number" value="1" min="1" max="10" class="form-control" id="inputBandGreen" style="width: 100%">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="inputBandNir" class="col-sm-3 control-label">近红外波段序号：</label>
                        <div class="col-sm-7">
                            <input type="number" value="1" min="1" max="10" class="form-control" id="inputBandNir" style="width: 100%">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="inputThreshold" class="col-sm-3 control-label">阈值：</label>
                        <div class="col-sm-7">
                            <input type="text" class="form-control" id="inputThreshold" placeholder="">
                        </div>
                    </div>
                    <div class="buttons" style="float: right">
                        ...
                        <a class="king-btn-demo king-btn king-round king-default king-disabled" title="下一步" id="button_next">下一步</a>
...
//                document.getElementById("button_preview").classList.add("king-disabled");
    ...
    function my_post() {
        var url = "http://47.100.193.191:8080/api/waterController/";
        var data = new FormData();
        var response = JSON.stringify({'code':201});
        var threshold = $("#inputThreshold").val();
        data.append('threshold', threshold);
        data.append('fname', getCookie('fname'));
        data.append('band_green', $("#inputBandGreen").val());
        data.append('band_nir',$("#inputBandNir").val());
        $.ajax({
            url: url,
            type: 'POST',
            cache: false,
        ...
        $("#button_next").click(function () {
            if (isReady()){
                ...
                if (JSON.parse(res)['code'] == 200){
                    setCookie('band_nir',$("#inputBandNir").val());
                    window.location.href = 'thirdStep';
                }
            }
        });
        $("#button_download").click(function () {
            ...
            if (isReady()){
                ...
                if (JSON.parse(res)['code'] !=200){
                    ...
                    var fname = (files.split('.')).join("_water.");
